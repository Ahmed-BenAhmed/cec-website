/**
 * Event interface that matches our database model
 */
interface Event {
  event_id: string;
  title: string;
...
export class EventsManager {
   ...
   */
  constructor(containerId: string, apiEndpoint: string) {
   ...
   */
  filterByStatus(status: 'all' | 'upcoming' | 'ongoing' | 'completed' | 'canceled'): void {
    if (!this.container) return;
   ...
   */
  private createEventCard(event: Event): HTMLElement {
